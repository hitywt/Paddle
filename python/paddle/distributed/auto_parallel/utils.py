...
def update_checkpoint_filelist(file_dir, latest_path, keep_checkpoint_max_num):
    ...
    checkpoint_dir_list = _get_checkpoint_directory(file_dir)
    print(
        f"debug current checkpoint list1 before: {checkpoint_dir_list}, latest_path: {latest_path}"
    )
    checkpoint_dir_list.sort(key=functools.cmp_to_key(sort_by_epoch_step))
    reserved_dir_list = []
    print(
        f"debug current checkpoint list1 after: {checkpoint_dir_list}, latest_path: {latest_path}"
    )
    keep = False
    for checkpoint_dir in checkpoint_dir_list:
    ...
    print(
        f"debug current checkpoint list2: {checkpoint_dir_list}, latest_path: {latest_path}"
    )
    if (
